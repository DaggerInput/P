...
for symbol in stocks:
    try:
        ...
        stock['DayIndex'] = np.arange(len(stock))
        ...
        future_dates = pd.date_range(start=stock['Date'].iloc[-1], periods=11, freq='B')[1:]

        fig, ax = plt.subplots(figsize=(10, 5))
        fig.patch.set_facecolor('#1e1e1e')
        ax.set_facecolor('#1e1e1e')

        ax.plot(stock['Date'], y, label='Actual Price', color='deepskyblue', linewidth=2, marker='o')
        ax.plot(future_dates, predicted_prices, label='Predicted Price', linestyle='--', color='tomato', linewidth=2)
